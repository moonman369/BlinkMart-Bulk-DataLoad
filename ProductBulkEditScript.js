...
const editProductCategories = async () => {
  const productDirectoryBasePath = process.env["PRODUCTS_PATH"];
  const baseUrl = process.env["SERVER_BASE_URL"];
  const productEditCategoriesEndpoint =
    process.env["PRODUCT_EDIT_CATEGORIES_ENDPOINT"];
  ...
  if (!productEditCategoriesEndpoint) {
    throw new Error("Product Category edit Endpoint not found in .env");
  }
  if (!loginEndpoint) {
  ...
  try {
    ...
    const productCategoryEditUrl = baseUrl + productEditCategoriesEndpoint;
        ...
        for (let subcategoryDir of subcategoryDirs) {
          ...
          for (let productFile of productSubDirs) {
            try {
              ...
              const response = await editProductCategory({
                cookieValue,
                editProductUrl: productCategoryEditUrl,
                name: productName,
                categories: [categoryName],
              });
              if (response.status === 200) {
                console.log(`Product edited successfully: ${productName}`);
              } else {
                console.error(`Failed to edit product: ${productName}`);
              }
            } catch (error) {
